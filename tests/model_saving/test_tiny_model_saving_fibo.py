...
class _TinyFiboTransformer(nn.Module):
    ...
    # FiboTransformer uses) at toy dimensions instead. Two of those leaves —
    # AdaLayerNormZeroSingle (inside FiboSingleTransformerBlock) and
    # TimestepEmbedder (inside BriaFiboTimestepProjEmbeddings), both under
    # src/mflux/models/flux/model/flux_transformer/ — take zero constructor args
    # and are hardcoded to dim=3072 themselves, so they stay full-size even here;
    # they are the real classes on the real save/load path, not stand-ins, so they
    # are kept rather than faked.
