...
- **`skip_quantization=True`** on any component (`krea2` and `qwen` each have one). That component stays unquantized through the roundtrip. This is fine and needs no handling — just don't be surprised its saved tree has no `scales`/`biases`, and note it in a comment.
